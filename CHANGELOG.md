...
* Optional `<ounit-id>` element has been added for each partner. The
  ...
  can now refer to an Organizational Unit, not only a HEI). See [this thread]
...
* All the elements describing the "local partner" were removed. The local
  ...
  `<partner>` element (the first of them, to be exact). See [this thread]
  (https://github.com/erasmus-without-paper/ewp-specs-api-iias/issues/13).
...
* Start and end dates were removed. They MAY return again, if we can define
  ...
  Discuss [here]
  (https://github.com/erasmus-without-paper/ewp-specs-api-iias/issues/10).
